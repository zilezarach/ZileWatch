...
api.interceptors.response.use(undefined, async (error: AxiosError) => {
  ...
  if (shouldRetry) {
    ...
    console.log(
      `[API] Retry ${config._retry}/${MAX_RETRIES} for ${config.url} after ${delay.toFixed(0)}ms`,
    );
...
function handleApiError(error: unknown, customMessage: string): never {
  if (axios.isAxiosError(error)) {
    ...
    throw new Error(
      `${customMessage}: ${serverMessage || error.message || "Unknown API error"}`,
    );
  }

  throw new Error(
    `${customMessage}: ${error instanceof Error ? error.message : "Unknown error"}`,
  );
}
...
export async function getMovieStreamingUrl(
  ...
  if (useFallback) {
    ...
    try {
      ...
      if (resp.data?.success && Array.isArray(resp.data.sources)) {
        const sources: FlixerSource[] = resp.data.sources;
        const subtitles = resp.data.subtitles || [];
        ...
        const m3u8Links = sources
          .filter(
            (source) =>
              source.status === "working" && source.url.includes(".m3u8"),
          )
          .map((source) => ({
      ...
      if (resp.data?.streamUrl) {
        ...
        return {
          ...
          selectedServer: {
            id: movieId,
            name: resp.data.method || resp.data.source || "TMDB",
          },
          availableQualities:
            resp.data.foundStreams?.map((url: string, index: number) => ({
              quality:
                index === 0
                  ? "1080P"
                  : index === 1
...
export async function getFlixerSources(
  ...
  try {
    ...
    if (resp.data?.success && Array.isArray(resp.data.sources)) {
      return resp.data.sources.filter(
        (s: FlixerSource) => s.status === "working",
      );
    }

    console.warn("[FLIXER] No sources found or unsuccessful response");
...
export async function getEpisodeStreamingUrlFallback(
  ...
  try {
    ...
    if (!sources.length) {
      throw new Error("No sources in extractor response");
    }
    ...
    const defaultStream =
      m3u8Links.find((l) => l.quality.toLowerCase() === "auto") || m3u8Links[0];
