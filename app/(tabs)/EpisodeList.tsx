...
export default function EpisodeListScreen() {
  ...
  const [selectedSourceType, setSelectedSourceType] = useState<SourceType>(useFallback ? "fallback" : "primary");
  ...
  const handleEpisodePress = (ep: EpisodeItem) => {
    ...
    if (selectedSourceType === "fallback" || selectedSourceType === "vidfast" || selectedSourceType === "wootly") {
  ...
  async function withRetries<T>(fn: () => Promise<T>, maxRetries: number = 3, delayMs: number = 2000): Promise<T> {
    let lastError: any;
  ...
  const startStreaming = async (ep: Episode) => {
    try {
      ...
      const sourceName =
        selectedSourceType === "vidfast" ? "Vidfast" : selectedSourceType === "wootly" ? "Wootly" : "Source One";
  ...
  const EpisodeRow = ({ item }: { item: EpisodeItem }) => (
    <TouchableOpacity style={styles.episodeItem} onPress={() => handleEpisodePress(item)} disabled={streamLoading}>
        ...
        <View style={styles.episodeInfo}>
          ...
          <View style={styles.sourceIndicator}>
            <Text style={styles.sourceIndicatorText}>
              {availableSources.find(s => s.id === selectedSourceType)?.name || "Source One"}
            </Text>
          </View>
          ...
          {selectedSourceType === "primary" && (
            <TouchableOpacity
              ...
              <Text style={styles.sourceButtonText}>Server: {selectedSource?.name || "Select"}</Text>
            </TouchableOpacity>
          )}
        </View>
      </View>
      ...
      <View style={styles.modalOverlay}>
        <View style={styles.modalContent}>
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <TouchableOpacity
                style={[styles.sourceItem, selectedSourceType === item.id && styles.selectedSourceItem]}
                onPress={() => handleSourceTypeSelect(item.id)}>
                <View>
                  <Text style={[styles.sourceText, selectedSourceType === item.id && styles.selectedSourceText]}>
                    {item.name}
                  </Text>
          ...
          />
          <TouchableOpacity style={styles.closeButton} onPress={() => setSourceModalVisible(false)}>
            <Text style={styles.closeText}>Close</Text>
          </TouchableOpacity>
        </View>
      </View>
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <TouchableOpacity style={styles.sourceSelectButton} onPress={() => setSourceModalVisible(true)}>
