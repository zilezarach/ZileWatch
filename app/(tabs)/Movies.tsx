  ...
  const handleSearch = async (query: string) => {
    ...
    try {
      ...
      if (cached) {
        ...
        const results = await streamingService.searchContent(query, contentType, useFallback);
        if (!isMounted.current) return;
        setMovies(results);
    ...
    return (
      <View style={styles.roleContainer}>
        <TouchableOpacity
          ...
          <Text style={[styles.toggleText, !useFallback && styles.toggleTextSelected]}>Source 1</Text>
        </TouchableOpacity>
        <TouchableOpacity
          onPress={() => setFallback(true)}
          style={[styles.toogleButton, !useFallback && styles.toggleSelected]}>
          <Text style={[styles.toggleText, !useFallback && styles.toggleTextSelected]}>Source 2</Text>
        </TouchableOpacity>
      </View>
    );
  };
