...
export default function Movies(): JSX.Element {
  ...
  // Removed useFallback state - always use Source 2 (TMDB)
  ...
  // Fetch homepage data - always use TMDB (Source 2)
  useEffect(() => {
    fetchHomeData();
  }, []);
  ...
  const fetchHomeData = async () => {
    ...
    try {
      setLoading(true);
      // Always fetch home data from TMDB via our service (Source 2)
  ...
  // Handle search - always use Source 2
  const handleSearch = async (query: string) => {
    ...
    try {
      setIsSearching(true);
      setLoading(true);
      const key = `search_${query}_${contentType}_source2`;
      ...
      if (cached) {
        ...
        // Always use Source 2 (fallback = true)
  ...
  // Handle item press - always pass Source 2 flag
  const handleItemPress = (item: any) => {
    ...
    if (itemType === "tvSeries") {
      navigation.navigate("SeriesDetail", {
        ...
        poster: item.poster,
        stats: item.stats,
        useFallback: true // Always use Source 2
      });
    } else {
  ...
  // Removed fallbackRoleToggle function since we're only using Source 2
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        <Text style={styles.appTitle}>{isSearching ? "Search" : "ZileStream"}</Text>
...
const styles = StyleSheet.create({
  ...
  // Removed source toggle styles (roleContainer, toogleButton, toggleText, toggleSelected, toggleTextSelected)
  tabsContainer: {
    flexDirection: "row",
    marginHorizontal: 16,
    marginBottom: 16,
