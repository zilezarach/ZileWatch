...
export default function Movies(): JSX.Element {
  ...
  const fetchTorrents = async (movieTitle: string) => {
    try {
      ...
      Alert.alert("Error", error.response?.data?.error || "Failed to fetch torrents. Please try again later.");
    } finally {
  ...
  const renderStream = () => (
    <View style={styles.videoContainer}>
      <Video source={{ uri: streamUrl }} style={styles.videoPlayer} controls resizeMode="contain" />
          ...
          renderItem={({ item }) => (
            <View style={styles.movieCard}>
              ...
              <View style={styles.movieDetails}>
                ...
                {torrents.length > 0 && (
                  <TouchableOpacity style={styles.buttonStream} onPress={() => playMovie(torrents[0].magnet)}>
