...
export default function MovieDetail(): JSX.Element {
  ...
  async function withRetries<T>(fn: () => Promise<T>, maxRetries: number = 4, delayMs: number = 4000): Promise<T> {
    let lastError: any;
  ...
  const fetchMovieDetails = useCallback(async () => {
    try {
      ...
      const errorMessage = err.response?.data?.message || err.message || "Failed to load movie details";
  ...
  const handleStreamAction = async (sourceType: "tmdb" | "vidfast" | "wootly") => {
    if (Platform.OS !== "web") {
    ...
    try {
      ...
      const fetchStream = async () => {
        ...
        switch (sourceType) {
          case "vidfast":
            return await streamingService.getMovieStreamingUrl(movieIdStr, initialSlug, false, true);
          case "wootly":
            return await streamingService.getMovieStreamingUrl(movieIdStr, initialSlug, useFallback, false, true);
          case "tmdb":
          default:
            return await streamingService.getMovieStreamingUrl(movieIdStr, initialSlug, true);
        }
      };
      ...
      navigation.navigate("Stream", {
        ...
        sourceName:
          sourceType === "vidfast" ? "Vidfast" : sourceType === "wootly" ? "Wootly" : info.selectedServer?.name,
        slug: initialSlug,
      ...
      Alert.alert("Streaming Error", `Unable to get ${sourceType} streaming information. Try another source.`, [
        { text: "OK" }
      ]);
    } finally {
  ...
  const renderRelatedContent = () => {
    if (!movieDetails?.related || movieDetails.related.length === 0) return null;
            ...
            <TouchableOpacity
              ...
              {item.stats?.rating && (
                <View style={styles.relatedRating}>
                  ...
                  <Text style={styles.relatedRatingText}>{item.stats.rating}</Text>
                </View>
              )}
            </TouchableOpacity>
          ))}
  ...
  const renderStreamingButton = (
    ...
    <TouchableOpacity
      ...
      {streamLoading && activeStreamSource === sourceType ? (
        ...
        <>
          <MaterialIcons name={icon || "play-arrow"} size={18} color="#FFF" style={styles.playIcon} />
  ...
  const getRatingFromStats = () => {
    const ratingStat = movieDetails?.stats?.find(
      stat => stat.name.toLowerCase().includes("rating") || stat.name.toLowerCase().includes("imdb")
    );
      ...
      <Animated.ScrollView
        ...
        <View style={styles.posterContainer}>
          ...
          <View style={styles.posterGradient}>
            <Text style={styles.movieTitle}>{movieDetails?.title || initialTitle}</Text>
            {getRatingFromStats() && (
        ...
        {movieDetails?.description && (
          <View style={styles.descriptionContainer}>
            ...
            <Text style={styles.descriptionText}>{movieDetails.description}</Text>
          </View>
        )}
